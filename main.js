...
function loadApp() {
  ...
  document.getElementById("mainApp").innerHTML = `
    <div class="section">
      ...
      <button onclick="exportToExcelByClass()">🏫 Xuất theo lớp</button>
      <button onclick="logout()">🔓 Thoát</button>
      <button onclick="showRankingTable()">🥇 Xếp hạng tổng</button>
      <button onclick="showRankingTable('week')">📆 Xếp hạng tuần</button>
      <button onclick="showRankingTable('month')">🗓️ Xếp hạng tháng</button>
...
function exportToExcel() {
  ...
  const BOM = "﻿";
  let csvContent = "Ngày,Người trực,Lớp,Danh mục,Điểm,Chú thích\n";
  records.forEach(r => {
...
function exportToExcelByClass() {
  ...
  const link = document.createElement("a");
  link.href = URL.createObjectURL(blob);
  link.download = `thi_dua_${lop}.csv`;
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
}

function getRankingByClass(period = "all") {
  const now = new Date();
  let fromDate = null;
  ...
  records.forEach(r => {
    ...
    if (!fromDate || rDate >= fromDate) {
      const score = parseInt(r.diem);
      if (!isNaN(score)) {
        if (!scores[r.lop]) scores[r.lop] = 0;
        scores[r.lop] += score;
      }
    }
  });
...
function showRankingTable(period = "all") {
  const ranking = getRankingByClass(period);
  let html = `<h4>🏆 Xếp hạng lớp (${period === "week" ? "Tuần" : period === "month" ? "Tháng" : "Tất cả thời gian"})</h4>`;
  html += `<table><thead><tr><th>Hạng</th><th>Lớp</th><th>Tổng điểm</th></tr></thead><tbody>`;
  ranking.forEach(([lop, total], index) => {
    html += `<tr><td>${index + 1}</td><td>${lop}</td><td>${total}</td></tr>`;
  });
  html += `</tbody></table>`;
